...
def eval_loss(w, b, x_list, gt_y_list):
    avg_loss_list = np.zeros(len(x_list))
     # loss function
...
def cal_step_gradient(batch_x_list, batch_gt_y_list, w, b, lr):
    ...
    db_list = batch_gt_y_list - pred_y_list
    dw_list = db_list * batch_x_list
    avg_dw = sum(db_list)/batch_size
    avg_db = sum(dw_list)/batch_size
...
def gen_sample_data():
    num_samples = 100
    w = np.random.randint(0, 10, num_samples) + np.random.rand(num_samples)		# for noise random.random[0, 1)
    b = np.random.randint(0, 5, num_samples) + np.random.rand(num_samples)
...
def train(x_list, gt_y_list, batch_size, lr, max_iter):
    ...
    batch_y = np.take(y_list, batch_idxs)
